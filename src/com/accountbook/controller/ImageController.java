...
public class ImageController {
	...
	public Object upload(@RequestParam("image") CommonsMultipartFile file, HttpServletRequest request,
	...
	public void getImage(HttpServletRequest request, HttpServletResponse response) {
		...
		// no-cache 必须先与服务器确认返回的响应是否被更改，然后才能使用该响应来满足后续对同一个网址的请求。因此，如果存在合适的验证令牌
		// (ETag)，no-cache 会发起往返通信来验证缓存的响应，如果资源未被更改，可以避免下载。
		// no-store 所有内容都不会被缓存到缓存或 Internet 临时文件中
		// must-revalidation/proxy-revalidation 如果缓存的内容失效，请求必须发送到服务器/代理以进行重新验证
		// max-age=xxx (xxx is numeric)单位秒 缓存的内容将在 xxx 秒后失效, 这个选项只在HTTP 1.1可用,
