...
public class AccountCalculator {
    ...
    public List<Account.PayResult> calc() throws CalculatorException{
        ...
        for (Account.Person p:mAccount.getPersons())
        ...
        for (Account.Person p:mAccount.getPersons()) {
            Account.Person.PayRule payRule = p.getPayRule();
            if(payRule == null) {
                noRulePersonCount++;
            ...
            switch (payRule.getNumberType()){
                case 0:
                    if(payRule.getNumber()>1)
                        throw new CalculatorException("您为"+p.getName()+"设置的支付类型是基于总支出百分比的值,取值范围[0~1]!");
                    //应缴金额
                    p.setShoudPay(mAccount.getPaidIn()*payRule.getNumber());
                    break;
                case 1:
                    if(payRule.getNumber()>mAccount.getPaidIn())
                        throw new CalculatorException(p.getName()+"支付的金额超过总支出!");
                    p.setShoudPay(payRule.getNumber());
                    break;
            }
        ...
        //======================step3=========================
        List<Account.Person> IN=new ArrayList<>();//需要收钱的人
        List<Account.Person> OUT=new ArrayList<>();//需要支出的人
     ...
     * @param OUT 付款者
     */
    private void calcResult(List<Account.Person> IN,List<Account.Person> OUT){
        Account.PayResult result=new Account.PayResult();
        result.setPayTarget(new ArrayList<Account.PayResult.PayTarget>());
        for(int i=0;i<IN.size();i++){
            //当前收钱的人
            Account.Person IN_P = IN.get(i);
            int n=0;
            //确定付款人的位置,n之前的人都已经付过了
            while (n<OUT.size() && OUT.get(n).getTempData()>=OUT.get(n).getShoudPay())
                n++;
            Account.Person OUT_P;
            while (n<OUT.size()){
                OUT_P = OUT.get(n++);

                Account.PayResult.PayTarget payTarget=new Account.PayResult.PayTarget();
                payTarget.setPayId(OUT_P.getId());
                payTarget.setGetId(IN_P.getId());

                double needMoney=IN_P.getPaidIn()-(IN_P.getShoudPay()+IN_P.getTempData());//还需收的钱.
                if(needMoney<0.001)
                    break;
                if(needMoney>=OUT_P.getShoudPay()-(OUT_P.getPaidIn()+OUT_P.getTempData())){
                    //收钱的人多付的钱大于付钱的人还需付的钱,把付钱人还需付的钱全部收来
                    double pay=OUT_P.getShoudPay()-(OUT_P.getPaidIn()+OUT_P.getTempData());
                    OUT_P.setTempData(OUT_P.getShoudPay());
                    IN_P.setTempData(IN_P.getTempData()+pay);
                    payTarget.setMoney(round(pay));

                }else if(needMoney<OUT_P.getShoudPay()-(OUT_P.getPaidIn()+OUT_P.getTempData())){
                    //收钱人多付的钱少于付钱人还需付的钱,向付钱人收自己多付的那部分
                    double pay=needMoney;
                    OUT_P.setTempData(OUT_P.getTempData()+pay);
                    IN_P.setTempData(IN_P.getTempData()+pay);
                    payTarget.setMoney(round(pay));
                }
        ...
        //初始化
        for (Account.Person p:IN)
            p.setTempData(0);
        for (Account.Person p:OUT)
            p.setTempData(0);
    }
     ...
     * @param j 位置
     */
    private void transposition(List<Account.Person> target,int i,int j){
        if(i==j)
            return;
        Account.Person temp=target.get(i);
